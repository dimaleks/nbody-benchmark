        ...
        const int spid, const float3 xsrc, const float3 usrc )
{
    ...
    const float rij2 = _xr * _xr + _yr * _yr + _zr * _zr + 1e-10f;
...
//#define ndsts 3
__forceinline__ __device__ float3 shfl_float3(const float3 x, const uint id)
{
    return make_float3(__shfl(x.x, id), __shfl(x.y, id), __shfl(x.z, id));
}

__forceinline__ __device__ float3 f4tof3(const float4 x)
{
    return {x.x, x.y, x.z};
}
...
template<int nDestParticles, int nchunks>
//__launch_bounds__(128, 8)
__global__ void nbodySharedPlusILPKernel(const float4 * const x, const float4 * const u, float4* a, int n)
{
    const int chunkId = blockIdx.y;
    const int startId = blockIdx.x * blockDim.x + threadIdx.x;
    const int dstStart = startId * nDestParticles;

    float3 xdst[nDestParticles], udst[nDestParticles], f[nDestParticles];
    extern __shared__ float4 cache[];
...
#pragma unroll
    for (int i=0; i<nDestParticles; i++)
        if (dstStart+i < n)
        {
            xdst[i] = f4tof3(x[dstStart + i]);
            udst[i] = f4tof3(u[dstStart + i]);
        }


    const int chSize = n / nchunks;
    ...
    for (int i = start; i < end; i+=blockDim.x)
    {
        if (i+threadIdx.x < n)
        {
            cache[2*threadIdx.x  ] = (x[i+threadIdx.x]);
            cache[2*threadIdx.x+1] = (u[i+threadIdx.x]);
        }
...
#pragma unroll 4
        ...
        {
            const float3 xsrc = f4tof3(cache[2*j]);
            const float3 usrc = f4tof3(cache[2*j+1]);

//#pragma unroll
            for (int d=0; d<nDestParticles; d++)
                f[d] += _dpd_interaction(dstStart+d, xdst[d], udst[d], i+j, xsrc, usrc);
        }
...
#pragma unroll
    for (int i=0; i<nDestParticles; i++)
        if (dstStart+i < n)
        {
            atomicAdd(&a[dstStart + i].x, f[i].x);
            atomicAdd(&a[dstStart + i].y, f[i].y);
            atomicAdd(&a[dstStart + i].z, f[i].z);
        }
}

// __launch_bounds__(128, 8)
// __global__ void squareCalcv2()
// {
...
//         if (dststart + sh < n)
//         {
//             xdst[sh] = f4tof3(x[dststart + sh]);
//             udst[sh] = f4tof3(u[dststart + sh]);
//         }
//     }
// 
//     for (uint i=0; i<n; i+=blockDim.x)
//     {
//         const uint src = i + thid;
//         const float3 myxsrc = f4tof3(x[src]);
//         const float3 myusrc = f4tof3(u[src]);
// 
// #pragma unroll 2
//         for (uint j=0; j<32; j++)
//         {
//             const uint srcThId = (thid + j) & 31;
//             const float3 xsrc = shfl_float3(myxsrc, srcThId);
//             const float3 usrc = shfl_float3(myusrc, srcThId);
// 
// #pragma unroll
//             for (uint sh = 0; sh < ndsts; sh++)
//             {
//                 if (dststart + sh < n)
//                 {
//                     const float3 f4 = _dpd_interaction(dststart+sh, xdst[sh], udst[sh], i + thid + srcThId, xsrc, usrc);
//                     printf("%d %d\n", i + srcThId, src);
//                     f[sh].x += f4.x;
//                     f[sh].y += f4.y;
//                     f[sh].z += f4.z;
...
#pragma omp for
    ...
    for (int i = 0; i<trials; i++)
    {
        ...
        const dim3 nblocks3(nblocks, nchunks, 1);
        ...
        cudaEventRecord(start);
        nbodySharedPlusILPKernel<ndsts, nchunks> <<< nblocks3, nthreads3, 2*nthreads*sizeof(float4) >>> (dp.x, dp.u, dp.a, ns);
        cudaEventRecord(stop);
    ...
    printf("  10%% / mean / 90%%  :  %.3f / %.3f / %.3f  MI/s (max %.3f Gflops/s)\n",
            records[trials / 10], avg, records[std::max(0, (9*trials) / 10 - 1)], 90.0 * records.back() / 1e3);
